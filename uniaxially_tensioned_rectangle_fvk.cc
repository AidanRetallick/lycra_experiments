...
namespace Params
{
  ...
  double relevant_energy_factor = 10000.0;
  ...
  {
    // Constant pressure
    pressure = p_mag * (x[1] - 0.5) * (x[1] + 0.5) * cos(6.0 * Pi * x[1]);
  }
...
public:
  ...
  void dump_it(ofstream& dump_file)
  {
    dump_file << Doc_steady_info.number() << " # steady step number"
              ...
              << Params::c_swell << " # swelling" << std::endl
...
void UnstructuredFvKProblem<ELEMENT>::apply_boundary_conditions()
{
  ...
  // We want completely free y-normal edges (0,2)
  // and pinned x-normal edges (1,3)
  Vector<unsigned> x_pinned_dofs = {0, 1, 2, 4, 5};
  Vector<unsigned>::iterator ptr;
  ...
  for (unsigned b = 0; b < n_bound; b++)
  {
    if (b % 2 == 1)
    {
      ...
      for (unsigned e = 0; e < nb_element; e++)
      {
        ...
        for (ptr = x_pinned_dofs.begin(); ptr < x_pinned_dofs.end(); ptr++)
        {
          // Pin x-normal edges
          el_pt->fix_out_of_plane_displacement_dof(
            *ptr, b, Params::get_null_fct);
        }
      }
    }
  }
                                                   ...
                                                   bool doc_unsteady)
{
  ...
  while (!STEADY)
  {
    ...
    {
      ELEMENT* el_pt = dynamic_cast<ELEMENT*>(Bulk_mesh_pt->element_pt(e));
      Vector<double> element_energies =
        el_pt->element_elastic_and_kinetic_energy();
    ...
    if (R * abs(kinetic_energy) < abs(elastic_energy) + 1.0e-16)
    {
      ...
      catch (OomphLibError& error)
      {
        if (R > 1.0e20)
        {
          ...
          oomph_info << "right to be. Giving up on damped solves..."
                     << std::endl;
          throw error;
        }
        else
        {
          oomph_info << "NOT STEADY ENOUGH. INCREASING THE KINETIC ENERGY "
                        "PROPORTION FROM "
                                                   ...
                                                   const std::string& comment)
{
  if (MPI_Helpers::communicator_pt()->my_rank() == 0)
  {
    ...
    Trace_file_nondim << Doc_steady_info.number() << " "
                      ...
                      << Params::p_mag << " " << Params::c_swell << " "
                      << w_centre[0] << endl;
    Trace_file_dim << Doc_steady_info.number() << " "
                   << Doc_unsteady_info.number() << " " << time() << " "
                   << Params::p_mag * Params::P_dim << " " << Params::c_swell
...
//=======================================================================
template<class ELEMENT>
void UnstructuredFvKProblem<ELEMENT>::delete_traction_elements(
...
int main(int argc, char** argv)
{
  ...
  CommandLineArgs::specify_command_line_flag("--L", &Params::L1);
  ...
  // Swelling increment
  double c_inc = 0.001;
  CommandLineArgs::specify_command_line_flag("--c", &c_inc);
  ...
  // (roughly as a multiple of length/thickness). Set to ceil(0.0*...) for
  // natural mesh handling at edges.
  Params::n_long_edge_nodes = ceil(0.1 * Params::L1 / Params::thickness) + 2;
  Params::n_short_edge_nodes = ceil(0.1 * Params::L2 / Params::thickness) + 2;

  // CREATE THE PROBLEM
  UnstructuredFvKProblem<DampedFoepplVonKarmanC1CurvedBellElement<4>> problem;
  ...
  // A flag to tell us whether we are perturbing our membrane or not.
  bool perturb_flag = true;
  ...
  {
    L += 0.05 * Params::L1;
    // Force displacement at x-normal boundaries.
    unsigned n_bound = problem.mesh_pt()->nboundary();
    for (unsigned b = 0; b < n_bound; b++)
    {
      // Stretch the right hand side out by enforcing displacement
      // along boundary 1
      if (b == 1)
      {
        const unsigned nb_nodes = problem.mesh_pt()->nboundary_node(b);
        for (unsigned n = 0; n < nb_nodes; n++)
        {
          Node* node_pt = problem.mesh_pt()->boundary_node_pt(b, n);
          node_pt->set_value(0, L / 2.0);
        }
      }
      // Stretch the left hand side out by enforcing displaement
      // along boundary 2
      if (b == 3)
      {
        const unsigned nb_nodes = problem.mesh_pt()->nboundary_node(b);
    ...
    // If we are perturbing the membrane, then perturb it!
    if (perturb_flag)
    {
      Params::p_mag += p_inc;
      problem.damped_solve(dt, epsilon, true);
    ...
    // Now solve with with extension and no pressure.
    problem.damped_solve(dt, epsilon, true);
    problem.doc_solution(true);

    // If we are perturbing, check whether the symmetry is broken
    // if it is, stop perturbing from here on.
    if (perturb_flag)
    {
      // Get the centre deflection
      // (as we are applying symmetric perturbation)
      Vector<double> origin(2, 0.0), s(2, 0.0);
      GeomObject* centre_element_pt;
      Vector<double> w_centre(1, 0.0);
      ...
      for (unsigned e = 0; e < n_element; e++)
      {
        dynamic_cast<DampedFoepplVonKarmanC1CurvedBellElement<4>*>(
          problem.mesh_pt()->element_pt(e))
          ->locate_zeta(origin, centre_element_pt, s);
        if (centre_element_pt != NULL)
        {
          w_centre = dynamic_cast<DampedFoepplVonKarmanC1CurvedBellElement<4>*>(
                       centre_element_pt)
                       ->interpolated_u_foeppl_von_karman(s);
          break;
        }
      } // End of for(...) element loop
      // Now check whether we have broken symmetry or not
      if (w_centre[0] > 1.0e-10)
      {
        // ...if so, stop perturbing the membrane
        perturb_flag = false;
      }
    } // End of if(...) perturbed check
  } // End of for(...) stretching loop
